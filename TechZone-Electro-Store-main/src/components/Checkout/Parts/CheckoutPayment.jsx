...
const getCardErrors = (cardData) => {
    const errors = {};
    if (cardData.name.trim().split(/\s+/).filter(Boolean).length < 2) {
        errors.name = 'Entrez le nom complet du titulaire.';
    }
    if (!passesLuhn(cardData.number)) {
        errors.number = 'Numero de carte invalide.';
    }
    if (!isValidExpiry(cardData.expiry)) {
        errors.expiry = 'Date expiration invalide ou expiree.';
    }
...
const CheckoutPayment = ({
    ...
    const needsNewCard = paymentMethod === 'card' && (isAddingNewCard || !user?.paymentMethods?.length);
    const cardErrors = useMemo(() => getCardErrors(cardData), [cardData]);
    const canContinue = paymentMethod !== 'card'
        || (needsNewCard ? Object.keys(cardErrors).length === 0 : Boolean(selectedCardId));
    const showCardErrors = attemptedNext && needsNewCard;
    const showSavedCardError = attemptedNext && paymentMethod === 'card' && !needsNewCard && !selectedCardId;
    const cardError = (field) => showCardErrors ? cardErrors[field] : null;
    ...
    return (
        <section className="space-y-10 animate-fade-right">
                    ...
                    <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/20 dark:shadow-none animate-fade-up">
                                ...
                                <div className="relative aspect-[1.6/1] bg-gradient-to-br from-slate-900 to-black rounded-3xl p-8 text-white overflow-hidden shadow-2xl flex flex-col justify-between group">
                                    ...
                                    <div className="flex justify-between items-start relative z-10">
                                        ...
                                        <div className="text-right">
                                            <p className="text-[8px] font-black uppercase tracking-widest text-slate-500">Partenaire premium</p>
                                            <p className="text-xs font-black uppercase italic tracking-tighter">CARTE PREMIUM</p>
                                        </div>
                                    </div>

                                    <div className="space-y-1 relative z-10">
                                        <p className="text-[8px] font-black uppercase tracking-[0.4em] text-slate-500">Numero de carte</p>
                                        <p className="text-base sm:text-lg font-display font-black tracking-[0.2em]">{cardData.number || '•••• •••• •••• ••••'}</p>
                                    </div>

                                    <div className="flex justify-between items-end relative z-10">
                                        <div>
                                            <p className="text-[7px] font-black uppercase tracking-widest text-slate-500">Titulaire</p>
                                            <p className="text-[10px] font-black uppercase tracking-widest">{cardData.name || 'VOTRE NOM'}</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-[7px] font-black uppercase tracking-widest text-slate-500">Expiration</p>
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Validation carte - passerelle bancaire requise</span>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="space-y-1.5">
                                            ...
                                            <input
                                                ...
                                                className={`w-full h-14 bg-slate-50 dark:bg-slate-950 border-2 rounded-2xl px-6 text-[10px] font-black focus:border-blue-600 transition-all outline-none dark:text-white uppercase tracking-widest ${cardError('name') ? 'border-rose-500' : 'border-slate-100 dark:border-slate-800'}`}
                                            />
                                            {cardError('name') && <p className="ml-4 text-[8px] font-black uppercase tracking-widest text-rose-500">{cardError('name')}</p>}
                                        </div>
                                        <div className="space-y-1.5">
                                            ...
                                            <input
                                                ...
                                                className={`w-full h-14 bg-slate-50 dark:bg-slate-950 border-2 rounded-2xl px-6 text-[10px] font-black focus:border-blue-600 transition-all outline-none dark:text-white tracking-[0.2em] ${cardError('number') ? 'border-rose-500' : 'border-slate-100 dark:border-slate-800'}`}
                                            />
                                            {cardError('number') && <p className="ml-4 text-[8px] font-black uppercase tracking-widest text-rose-500">{cardError('number')}</p>}
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="space-y-1.5">
                                                ...
                                                <input
                                                    ...
                                                    className={`w-full h-14 bg-slate-50 dark:bg-slate-950 border-2 rounded-2xl px-6 text-[10px] font-black focus:border-blue-600 transition-all outline-none dark:text-white ${cardError('expiry') ? 'border-rose-500' : 'border-slate-100 dark:border-slate-800'}`}
                                                />
                                                {cardError('expiry') && <p className="ml-4 text-[8px] font-black uppercase tracking-widest text-rose-500">{cardError('expiry')}</p>}
                                            </div>
                                            <div className="space-y-1.5">
                                                ...
                                                <input
                                                    ...
                                                    maxLength="4"
                                                    value={cardData.cvc}
                                                    onChange={(e) => setCardData({ ...cardData, cvc: onlyDigits(e.target.value).slice(0, 4) })}
                                                    className={`w-full h-14 bg-slate-50 dark:bg-slate-950 border-2 rounded-2xl px-6 text-[10px] font-black focus:border-blue-600 transition-all outline-none dark:text-white ${cardError('cvc') ? 'border-rose-500' : 'border-slate-100 dark:border-slate-800'}`}
                                                />
                                                {cardError('cvc') && <p className="ml-4 text-[8px] font-black uppercase tracking-widest text-rose-500">{cardError('cvc')}</p>}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2 text-[8px] font-black uppercase tracking-widest text-slate-400 mt-4">
                                        ...
                                        Test de format uniquement, sans debit bancaire.
                                    </div>
                                </div>
                            </div>
                        )}
                        {showSavedCardError && (
                            <p className="text-[9px] font-black uppercase tracking-widest text-rose-500">
                                Choisissez une carte enregistree ou ajoutez une nouvelle carte.
                            </p>
                        )}
                    </div>
                )}
            ...
            <div className="flex gap-4">
                ...
                {paymentMethod !== 'paypal' && (
                    <button onClick={handleNext} className={`flex-1 h-16 bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-blue-600/20 hover:scale-[1.02] active:scale-95 transition-all ${!canContinue ? 'opacity-70' : ''}`}>Suivant : Confirmation</button>
                )}
            </div>
        </section>
    );
