...
import { isValidMoroccanPhone } from '../../../utils/helpers';

const CheckoutDelivery = ({ formData, setFormData, onNext }) => {
    const [attempted, setAttempted] = useState(false);
    const isPhoneValid = isValidMoroccanPhone(formData.phone);
    const canContinue = formData.name.trim() && isPhoneValid && formData.address.trim();
    const showPhoneError = attempted && !isPhoneValid;
    ...
    return (
        <section className="space-y-10 animate-fade-right">
            <div className="flex items-center gap-4 border-b border-slate-100 dark:border-slate-800 pb-6">
                ...
                <h3 className="text-xl font-black uppercase tracking-tighter dark:text-white font-display">Informations de livraison</h3>
            </div>
                ...
                <div className="space-y-2 group">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className={`w-full h-16 bg-white dark:bg-slate-950 border-2 rounded-2xl pl-14 pr-6 text-sm font-bold focus:border-blue-600 transition-all outline-none dark:text-white ${showPhoneError ? 'border-rose-500' : 'border-slate-100 dark:border-slate-800'}`}
                            placeholder="06 XX XX XX XX"
                        />
                    </div>
                    {showPhoneError && (
                        <p className="ml-4 text-[9px] font-black uppercase tracking-widest text-rose-500">
                            Entrez un numero marocain valide: 06/07/05 ou +212.
                        </p>
                    )}
                </div>
            </div>
